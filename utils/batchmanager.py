...
class BatchManager:
    ...
    def ManageOutputDir( self ):
        ...
        if( os.path.isdir(self.outputDir_) == True and os.listdir(self.outputDir_) ):
            print "This directory already exists, it will be overwritten now!"
            print "To change this back to ask for confirmation go to batchManager.py in heppy/utils"
            input = 'y'
...
#                    input = raw_input( 'The directory ' + self.outputDir_ + ' exists. Are you sure you want to continue? its contents will be overwritten [y/n]' )
    ...
    def SubmitJob( self, jobDir ):
        ...
        #os.system(  'echo '+self.options_.batch+' >> ~/big_sub_test.txt' )
    ...
    def RunningMode(self, batch):
        ...
        if batchCmd == 'bsub':
            ...
            if onPSI:
                ...
                print 'running on IIHE : %s from %s' % (batchCmd, hostName)
                return 'IIHE'
            elif onIC:
