...
def batchScriptIIHE(jobDir):
   '''prepare a IIHE version of the batch script'''


   script = """#!/bin/bash
# Choose 0.8 or 0.9, whichever works but no need to keep both! 
#echo 'sourcing FCCSW v0.9'
#source /cvmfs/fcc.cern.ch/sw/views/releases/0.9.1/x86_64-slc6-gcc62-opt/setup.sh
echo 'sourcing FCCSW v0.8'
source /cvmfs/fcc.cern.ch/sw/0.8.1/init_fcc_stack.sh
echo 'sourcing the heppy directory'
cd ~/testFCC/Nicolo/heppy/
source ./init.sh
cd {jdir}
echo 'running'
python {looper} pycfg.py config.pck --options=options.json
...
echo 'sending the job directory back'
mv Loop/* ./
""".format(jdir = jobDir, looper=looper.__file__)
   return script
...
class MyBatchManager( BatchManager ):
   ...
   def PrepareJobUser(self, jobDir, value ):
      ...
      if mode == 'LXPLUS':
         ...
         scriptFile.write( batchScriptIIHE(jobDir))
      elif mode == 'IC':
